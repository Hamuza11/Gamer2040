...
def background():
    ...
    while True:
        ...
        pgb.create_text("ST57600          "+version,10,225,ttcolor)
        options=["Image Background","Color Background", "Exit"]
                ...
                while True:
                    ...
                    if pgb.button_A():
                        if opt==len(options)-1:
                            ...
                            with open("background.conf","w") as w:
                                w.write(f"/backgrounds/{bgs[opt+(page*4)]}")
                                bgcolor=bgcolors[8]
                                bindex=8
                                bgcolor565=PicoGameBoy.color(*bgcolor)
                                if sum(bgcolor)<426:
...
while True:
    ...
    options=["Settings","Change Volume", "Change Background", "Data Upload Mode", "Toggle Animation: "+str(animated), "Exit"]
    if tcolor==0:
        ottcolor=white
    else:
        ottcolor=black
    ...
    if pgb.button_A():
        ...
        if opt==3:
            ...
            pgb.create_text("upload mode press",-1,192,PicoGameBoy.color(255,255,255))
            pgb.create_text("A.",-1,204,PicoGameBoy.color(255,255,255))
            pgb.show()
