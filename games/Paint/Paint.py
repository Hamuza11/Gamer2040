...
rename("/main.py", "/games/Paint/Paint.py")
                ...
                while go:
                    ...
                    if pgb.button_A():
                        ...
                        if afree-tot-5000<0:
                            ...
                            sleep(2)
                            return
                        with open("/games/Paint/D", "a") as w:
                            w.write("---PICOBOYFILELIST---"+file.replace(" (Used)","")+".pbd")
                        del w
                        with open("/backgrounds/"+file.replace(" (Used)","")+".pbd", "w") as e:
                            e.write("")
                        with open("/backgrounds/"+file.replace(" (Used)","")+".pbd", "a") as f:
                            for row in drawing:
                ...
                while True:
                    ...
                    if pgb.button_A():
                        ...
                        if not "(Used)" in file:
                            ...
                            try:
                                drawing=[]
                                with open("/backgrounds/"+file+".pbd") as f:
                                    for i in range(30):
                                        comp=[]
                ...
                while True:
                    ...
                    if pgb.button_A():
                        ...
                        if not "(Used)" in file:
                            ...
                            pgb.show()
                            sleep(0.5)
                            with open("/games/Paint/D", "r") as f:
                                lines = f.readlines()
                            del f
                            with open("/games/Paint/D", "w") as f:
                                for line in lines:
                                    new_line = line.replace("---PICOBOYFILELIST---"+file+".pbd", "")
                                    f.write(new_line)
                            
                            remove("/backgrounds/"+file+".pbd")
                            del file
                        del files
                        del options
                        po=listdir("/backgrounds/")
