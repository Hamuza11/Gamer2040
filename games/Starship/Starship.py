...
rename("main.py","/games/Starship/Starship.py")
...
while True:
    ...
    if pgb.button_Home():
        ...
        machine.reset()
        break
    pgb.load_image("/games/Starship/starship_title.bin")
    pgb.show()
    if ticks_diff(ticks_ms(), now) > 200:
        ...
        now = ticks_ms()
    if pgb.button_start():
            x=open("/games/Starship/highscoresStarship.sc", "r")
            scores=x.read()
            x.close()
            del x
            scores=scores.split("\n")
...
0xF0]
            ...
            while True:
                if pgb.button_Home():
                    with open("/games/Starship/highscoresStarship.sc", "r") as s:
                        scores=s.read().split("\n")
                    ...
                    with open("/games/Starship/highscoresStarship.sc", "w+") as w:
                        w.write("\n".join(newscores[:10]))
                    del newscores
                    del scores
                    homebootstop=open("/noboot", "w")
