...
os.rename("/main.py", "/games/Breakout/Breakout.py")
...
def append_to_board(score):
    with open("/games/Breakout/highscoresBreakout.sc", "r") as s:
        scores=s.read().split("\n")
    ...
    with open("/games/Breakout/highscoresBreakout.sc", "w+") as w:
        w.write("\n".join(newscores[:10]))

def view_scores():
    x=open("/games/Breakout/highscoresBreakout.sc", "r")
    scores=x.read()
    x.close()
    del x
    scores=scores.split("\n")
...
def title_screen():
    ...
    while True:
        ...
        if pgb.button_start():
            ...
            break
        pgb.load_image("/games/Breakout/breakout_title.bin")
        pgb.show()
