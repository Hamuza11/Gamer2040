...
rename("/main.py", "/games/Pac Man/Pac Man.py")
...
lscore=0
lives=3
with open("/games/Pac Man/sprite_pellet.bin","rb") as sprt:
    pgb.add_sprite(bytearray(sprt.read()), 5,5)
with open("/games/Pac Man/sprite_blinky.bin","rb") as sprt:
    pgb.add_sprite(bytearray(sprt.read()),16,16)
with open("/games/Pac Man/sprite_clyde.bin","rb") as sprt:
    pgb.add_sprite(bytearray(sprt.read()),16,16)
with open("/games/Pac Man/sprite_inky.bin","rb") as sprt:
    pgb.add_sprite(bytearray(sprt.read()),16,16)
with open("/games/Pac Man/sprite_pinky.bin","rb") as sprt:
    pgb.add_sprite(bytearray(sprt.read()),16,16)
with open("/games/Pac Man/sprite_panicked.bin","rb") as sprt:
    pgb.add_sprite(bytearray(sprt.read()),16,16)
        ...
        if alive:
            if playerstate:
                ...
                if dt%70==0:
                    ...
                    soundalt=True
                    playerspeed=4
            pgb.load_image("/games/Pac Man/pacmanbg.bin")
...
def view_scores():
    x=open("/games/Pac Man/highscoresPac Man.sc", "r")
    scores=x.read()
    x.close()
    del x
    scores=scores.split("\n")
...
while True:
    if pgb.button_Home():
        ...
        machine.reset()
        break
    pgb.load_image("/games/Pac Man/pacman_title.bin")
    pgb.show()
...
while True:
    if new_level():
        ...
        with open("/games/Pac Man/highscoresPac Man.sc", "r") as s:
            scores=s.read().split("\n")
        ...
        with open("/games/Pac Man/highscoresPac Man.sc", "w+") as w:
            w.write("\n".join(newscores[:10]))
        while True:
            pgb.fill(BLACK)
