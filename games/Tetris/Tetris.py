...
os.rename("/main.py", "/games/Tetris/Tetris.py")
...
def append_to_board(score):
    try:
        with open("/games/Tetris/highscoresTetris.sc", "r") as s:
            scores=s.read()
        scores=scores.split("\n")
        ...
        with open("/games/Tetris/highscoresTetris.sc", "w+") as w:
            w.write("\n".join(newscores[:10]))
    except:
        return

def view_scores():
    x=open("/games/Tetris/highscoresTetris.sc", "r")
    scores=x.read()
    x.close()
    del x
    scores=scores.split("\n")
...
def title_screen():
    ...
    while True:
        if pgb.button_Home():
            ...
            machine.reset()
            break
        pgb.load_image("/games/Tetris/tetris_title.bin")
        pgb.show()
...
def main_game():
    ...
    while True:
        ...
        if pgb.button_start():
            pause_screen()
        if level<=5:
            if lines >= 30 +(level*30):
                level+=1
        else:
            if lines >= 50 +(level*50):
                level+=1
        dx=0
        dy=1
        rotate=False
        ...
        if (pgb.button_A() or pgb.button_B()) and not n==6:
            ...
            last_button="UP"
                ...
                dx=-1
                pauseleft=True
        elif pgb.button_right() and not pauseright and not skip:
                last_button="RIGHT"
                dx=1
                pauseright=True
        ...
        if skip:
            skip=False
        if pgb.button_down():
            delay=int(50-level*.166)
            if int(50-level*.166)<0:
                delay=0
        else:
            delay=int(500-level*16.6)
            if int(500-level*16.6)<0:
                delay=0
